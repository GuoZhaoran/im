...
func (service *ContactService) SearchComunity(userId int64) ([]model.Community) {
	...
	DbEngine.Where("ownerid = ? and cate = ?", userId, model.ConcatCateComunity).Find(&conconts)
...
func (c *ContactService) SearchCommunityByName(cname string) (model.Community) {
	com := model.Community{}
	DbEngine.Where("name = ?", cname).Get(&com)
	return com
}
...
func (c *ContactService) SearchFriend(userId int64) ([]model.User) {
	...
	objIds := make([]int64, 0)
	DbEngine.Where("ownerid = ? and cate = ?", userId, model.ConcatCateUser).Find(&friends)
...
func (c *ContactService) SearchFriendByName(mobile string) (model.User) {
	user := model.User{}
	DbEngine.Where("mobile = ?", mobile).Get(&user)
	return user
}
